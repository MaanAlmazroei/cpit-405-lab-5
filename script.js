...
document.addEventListener("DOMContentLoaded",() => {
    ...
    function filterByName() {
        const inputWords = searchWord.value.toLowerCase();
        filterTable(0, inputWords);
    }
    function filterByTeam() {
        ...
        if (selectedTeam ==="all") {
            ...
            filterTable(1,selectedTeam);
        }
    }

    function filterTable(columnIndex, filterTerm) {
        const rows = tableBody.querySelectorAll("tr");
        rows.forEach(row => {
            const cell = row.getElementsByTagName("td")[columnIndex];
            const cellValue =cell.textContent.toLowerCase();
            row.style.display = cellValue.includes(filterTerm) ? "" : "none";
        });
    }
    ...
    searchWord.addEventListener("input",filterByName);
